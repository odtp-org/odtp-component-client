...
class MongoManager:
    ...
    def add_digital_twin(self, userRef, digital_twin_data):
        ...
        digital_twin_id = digital_twins_collection.insert_one(
            digital_twin_data
        ).inserted_id

        # Add digital twin reference to user
        self.db.users.update_one(
            {"_id": userRef}, {"$push": {"digitalTwins": digital_twin_id}}
        )
    ...
    def append_execution(self, digital_twin_id, execution_data):
        ...
        self.db.digitalTwins.update_one(
            {"_id": digital_twin_id}, {"$push": {"executions": execution_id}}
        )
    ...
    def append_step(self, execution_id, step_data):
        ...
        self.db.executions.update_one(
            {"_id": execution_id}, {"$push": {"steps": step_id}}
        )
    ...
    def append_log(self, step_id, log_data):
        ...
        steps_collection.update_one(
            {"_id": ObjectId(step_id)}, {"$push": {"logs": log_data}}
        )

    def append_logs(self, step_id, log_data_list):
        steps_collection = self.db["steps"]
        steps_collection.update_one(
            {"_id": ObjectId(step_id)}, {"$push": {"logs": {"$each": log_data_list}}}
        )

    def update_result(self, result_id, output_id):
        results_collection = self.db["results"]
        results_collection.update_one(
            {"_id": ObjectId(result_id)}, {"$push": {"output": output_id}}
        )
    ...
    def get_all_collections_as_json_string(self):
        ...
        return json.dumps(
            all_data, indent=2, default=str
        )  # default=str to handle datetime and ObjectId
    ...
    def print_all_collections_as_json(self):
        ...
        for collection_name in self.db.list_collection_names():
            ...
            for doc in cursor:
                ...
                )  # default=str is added to handle datetime and ObjectId
    ...
    def export_all_collections_as_json(self, filename):
        ...
        with open(filename, "w") as json_file:
            json.dump(
                all_data, json_file, indent=2, default=str
            )  # default=str to handle datetime and ObjectId
    ...
    def get_digital_twins_by_user_id(self, user_id_str):
        ...
        for doc in cursor:
            doc["_id"] = str(
                doc["_id"]
            )  # Convert ObjectId to string for pandas compatibility
    ...
    def print_logs_by_indices(self, twin_index, execution_index, step_index):
        ...
        digital_twin = (
            self.db.digitalTwins.find().sort("_id", 1).skip(twin_index).limit(1).next()
        )

        try:
            # Navigate to the logs using the given execution index
            logs = digital_twin["executions"][execution_index]["steps"][step_index][
                "logs"
            ]
        except (IndexError, KeyError):
            print(
                f"No logs found for execution {execution_index} of digital twin {twin_index}."
            )
...
def main():
    ...
    with open("/odtp/odtp-logs/log.txt", "r", encoding="utf-8") as f:
